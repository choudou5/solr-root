          ...
          (
            'cores_load_template',
            {
              ...
              callback : function()
              {
				...
				$('button.dialog').die( 'click' ).live('click', function( event ){
					var self = $( this );
					self.toggleClass( 'open' );
					$('.dialog.'+self.attr('id')).trigger( 'open' );
					return false;
				  }
				);
				$('button.reset').die( 'click' ).live('click', function( event ){
					var error_elem = $( '.error', dialog.filter( ':visible' ) );
					$( 'span', error_elem ).text('');
					error_elem.hide();
					$(this).closest('div.dialog' ).trigger( 'close' );
				  }
				);
				//绑定 关闭事件
				dialog.die( 'close' ).live('close', function( event ){
					dialog.hide();
				...
				//上传文件 change 事件
				$(document).on("change", "#uploadConf_file", function() {
					$('#uploadConf_shards').val('');
					$('#uploadConf_confName').val('');
				  	$('#uploadConf_deploy').prop('checked', false);
				  	
					var filePath = $(this).val();
				  	if(filePath != '' && filePath.endWith('.zip')){
				  		if($('#cloud.global').is(':visible')){
				  			$('.updateConfAutoDeploy').show();
				  		}
				  		$('.updateConfName').hide();
				  	}else{
				  		$('.updateConfAutoDeploy').hide();
				  		if($('#cloud.global').is(':visible')){
				  			$('.updateConfName').show();
		                }
				  	}
				...
				//上传 core 配置文件
				$('#uploadConfForm', dialog).ajaxForm({
					  url : app.config.solr_path + "/custom" + '?wt=json&indexInfo=false&action=upload_conf',
					  dataType : 'json',
					  beforeSubmit : function( array, form, options ){
						$( 'button[type="submit"] span', form ).addClass( 'loader' );
						if($('#cloud.global').is(':visible')){
		                	$('#coresModel').val("cloud");
		                }
					  },
					  success : function( response, status_text, xhr, form ){
						console.log("success");
					  ...
					  error : function( xhr, text_status, error_thrown ){
						...
						var msg = "上传失败！";
						if(resp != null){
							var response = eval( 'response = ' + resp + ';' );
							...
							if(status != 0){
								...
								var error_elem = $( '.error', dialog.filter( ':visible' ) );
								error_elem.show();
								$( 'span', error_elem ).text(msg);
							}else{
								$( 'button.reset', $('#uploadConfForm')).trigger( 'click' );
								jQnotice('上传成功！', {top: 100, left: 22});
							}
						}

					  },
					  complete : function(){
						$( 'button span.loader').removeClass( 'loader' );
					  }
					}
				  );
